...
using StreamForge.AppCore.Plugins;
...
public class OutOfTreeKindTests
{
    ...
    public void A_missing_plugin_directory_is_silence_not_an_error()
    {
        ...
        // alone a failure.
        Assert.Empty(StreamForgePlugins.LoadFrom(Path.Combine(Path.GetTempPath(), $"sf-plugins-{Guid.NewGuid():N}")));
    }
    ...
    public void A_file_that_is_not_a_managed_assembly_is_reported_and_skipped()
    {
        ...
        try
        {
            ...
            var report = StreamForgePlugins.LoadFrom(dir);
    ...
    private sealed class DuplicateKindPlugin : IStreamForgePlugin
    {
