...
/// <c>StreamsForge.AppCore.Connectors.ConnectorRowCoercion</c>'s <b>Null</b> policy specifically (the
...
public static class CrdtProjector
{
    ...
    /// string literals rather than referencing <c>StreamsForge.Engine.PublicApi</c> or
    /// <c>StreamsForge.AppCore.Sinks.SinkStepGuard</c> directly: this project is deliberately
    /// runtime-agnostic and Engine-free (matching <c>StreamsForge.Connectors.Fix</c>'s own
    ...
    /// <c>StreamsForge.Connectors.Database.CdcStamp</c> uses (<c>_op</c> ∈ {"c","u","d"}, <c>_weight</c>
    ...
    /// project reference on <c>StreamsForge.Connectors.Database</c> — that project pulls a live-database
    ...
    /// <summary>
    /// The platform's ONE real key retraction (<c>StreamsForge.AppCore.Ingest.IngressRowAcceptance</c>'s
