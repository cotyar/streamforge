...
///
/// <para><see cref="NamedEndpoints"/> is process-wide, but this test project (<c>StreamForge.Connectors.Database.Tests</c>)
/// runs as its own test host process, separate from <c>StreamForge.AppCore.Tests</c> and
/// <c>StreamForge.Connectors.Fix.Tests</c> — so the only classes that can race this file's use of it are
