...
public sealed record TableDelta(EventRecord Row, long Weight)
{
    ...
    /// every key it ever saw — see StreamsForge.Host.Grains.TableHistoryGrain). Consumers that do not care
...
public sealed partial class TablePlan
{
    ...
    /// StreamsForge.Engine.Tests.TableZSetTests.GlobalAggregateWithoutGroupByIsASingleGroup). A plan WITH a
    ...
    /// StreamsForge.Host.Grains.TableKeyFields to distinguish "one global group" ([] on the wire) from
...
public sealed partial class TableExecutor
{
    ...
    /// of it dead weight for a caller that only ever sees the wire shape (StreamsForge.Host's
