...
/// (<c>Sql/Parser.cs</c>).
///
/// Public and keyed on the Engine's own <see cref="FieldKind"/> — not <c>StreamsForge.Abstractions</c>'
/// <c>FieldType</c> — precisely so it can ALSO be the single implementation behind
/// <c>StreamsForge.AppCore.Ingest.FieldValueCoercion.TryCoerce</c>, which already implements the
...
/// <c>StreamsForge.Abstractions</c>/Contracts), so once that call site maps <c>FieldType</c> →
...
public static class FieldValueConversion
{
    ...
    /// <summary>Exact behavioral port of <c>StreamsForge.AppCore.Ingest.RowTimestamp.Resolve</c> — kept
