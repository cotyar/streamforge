...
namespace StreamForge.AppCore.Plugins;
...
/// </summary>
public interface IStreamForgePlugin
{
...
/// <summary>
/// Loads <see cref="IStreamForgePlugin"/> implementations out of a directory of assemblies, so a
...
/// with an explicit <see cref="IStreamForgePlugin.Register"/> call, and the only thing discovered is which
...
/// share its <c>StreamForge.AppCore</c>/<c>StreamForge.Contracts</c> types — a transport in an isolated
...
/// </summary>
public static class StreamForgePlugins
{
    ...
    /// fails to load, a plugin whose constructor throws, and a <see cref="IStreamForgePlugin.Register"/>
    ...
    private static List<string> LoadAssembly(string file)
    {
        ...
        [
            .. assembly.GetExportedTypes()
                .Where(t => typeof(IStreamForgePlugin).IsAssignableFrom(t) && t is { IsAbstract: false, IsInterface: false })
    ...
    private static string Activate(Type type)
    {
        try
        {
            var plugin = (IStreamForgePlugin)Activator.CreateInstance(type)!;
            plugin.Register();
