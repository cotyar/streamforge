...
/// former Program.cs so both runtimes serve byte-identical responses. <see cref="AddStreamsForgeApi"/>
/// registers services (call from the host's builder.Services); <see cref="MapStreamsForgeApi"/> wires
/// middleware + endpoint routes (call once <c>WebApplication</c> is built), taking a
/// <see cref="StreamsForgeApiOptions"/> for the handful of host-specific facts (protos dir, gRPC port +
/// static service list, docs file, SPA dist path).
/// </summary>
public static class StreamsForgeApiExtensions
{
    private const string SpaCorsPolicy = "SpaDev";

    public static void AddStreamsForgeApi(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddSingleton(sp => new PermissionResolver(
            sp.GetRequiredService<StreamsForge.Abstractions.IAccessPolicyFacade>(),
        ...
        // call AddStreamsForgeApi, so this is the one place that reaches Orleans and Dapr at once.
        //
        ...
        services.AddSingleton<IChatApprovalFiler>(sp => new ApprovalStoreChatFiler(
            () => sp.GetService<StreamsForge.Abstractions.IApprovalFacade>(),
            approvalOptions,
        ...
        if (auditEnabled)
        {
            services.AddSingleton<IHostedService>(sp => new AuditWriterService(
                ...
                () => sp.GetService<StreamsForge.Abstractions.IAuditFacade>(),
        ...
        services.AddAuthorizationBuilder()
            ...
            .AddPolicy("Editor", p =>
            {
                ...
                p.AddRequirements(new LegacyPolicyRequirement(
                    ...
                    [StreamsForge.Abstractions.BuiltInRoles.Editor, StreamsForge.Abstractions.BuiltInRoles.Admin]));
            })
            .AddPolicy("Admin", p =>
            {
                p.RequireAuthenticatedUser();
                p.AddRequirements(new LegacyPolicyRequirement(
                    ...
                    [StreamsForge.Abstractions.BuiltInRoles.Admin]));
            });
        ...
        services.AddOpenApi(options =>
        {
            ...
            {
                document.Info.Title = "StreamsForge API";
        ...
        services.AddScoped(sp =>
        {
            ...
            return new GeminiChatService(
                ...
                model,
                apiKey,
                sp.GetRequiredService<StreamsForge.Abstractions.ICatalogFacade>(),
                sp.GetRequiredService<StreamsForge.Abstractions.ITableReadFacade>(),
                sp.GetRequiredService<StreamsForge.Abstractions.ITableHistoryFacade>(),
    ...
    public static void MapStreamsForgeApi(this WebApplication app, StreamsForgeApiOptions options)
    {
        ...
        // anonymous caller cannot use X-StreamsForge-Environment to enumerate which environments exist on
                 ...
                 new[] { ("tables", "id"), ("pipelines", "id"), ("sources", "name") })
        {
            app.MapScalarApiReference($"/scalar/{segment}/{{{parameter}}}", (o, httpContext) =>
            {
                ...
                o.Title = $"StreamsForge — {segment}/{key}";
                o.Theme = ScalarTheme.Kepler;
