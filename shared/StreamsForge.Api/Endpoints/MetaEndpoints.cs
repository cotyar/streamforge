...
/// <summary>One of the two hand-authored static .proto files StreamsForge ships (Protos/), returned
...
/// service list are host-specific facts carried by <see cref="StreamsForgeApiOptions"/> rather than
...
{
    private static readonly string[] StaticProtoFileNames = ["streamsforge.proto", "streamsforge_dynamic.proto"];

    public static void MapMetaEndpoints(this WebApplication app, StreamsForgeApiOptions options)
    {
        ...
        // StreamsForge.Api, so GetExecutingAssembly() here IS that assembly, unlike ProtoFileBuilder's
        // same-shaped fallback (StreamsForge.AppCore) which answers a different question.
        var assemblyVersion =
            Assembly.GetExecutingAssembly().GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion
            ?? Assembly.GetExecutingAssembly().GetName().Version?.ToString()
            ?? "dev";
        // Plan 016 wave 5: honest capability signalling — this instance serves gRPC iff it maps static
        // gRPC services at all (GrpcStaticServices is how StreamsForgeApiOptions already tells the two
