...
/// <summary>
/// Recovers <c>(List&lt;FieldDef&gt;, FieldNumberMap)</c> for an entity's row schema from a StreamsForge
...
/// with the exact generator banner and has <c>streamsforge.dynamic.v1</c> as its package — and is a
...
/// PRESENT (as extra confidence this really is a StreamsForge dynamic-entity file) but not otherwise
...
/// prints a disambiguating trailing comment on every scalar field ("StreamsForge: Long" vs "StreamsForge:
...
{
    private const string BannerPrefix = "// StreamsForge generated .proto";
    ...
    /// map, or (null, null, diagnostics) when the text isn't a recognizable StreamsForge-generated
    ...
    public static (List<FieldDef>? Fields, FieldNumberMap? Numbers, IReadOnlyList<string> Diagnostics) Parse(string protoText)
    {
        ...
        {
            diagnostics.Add(
                "Not a StreamsForge-generated proto file: expected the generator banner " +
                $"('{BannerPrefix} ... {BannerMarker}...') as the first line. This parser only accepts " +
                "/api/{kind}/{key}/proto downloads from a StreamsForge instance, not arbitrary third-party " +
        ...
        if (!lines.Contains($"message {rootName}Event {{") || !lines.Contains($"message {rootName}Delta {{"))
        {
            diagnostics.Add(
                ...
                "does not look like a StreamsForge dynamic-entity proto (DescriptorFactory always emits " +
    ...
    private static FieldType ClassifyType(string typeText, string? comment, string path, List<string> diagnostics, out bool isNestedJson)
    {
        ...
        switch (typeText)
        {
            ...
            case "int64":
                ...
                {
                    diagnostics.Add($"Field '{path}' is int64 with no recognizable StreamsForge type comment - defaulting to Long.");
                }
                return FieldType.Long;
            case "sint64":
                diagnostics.Add($"Field '{path}' uses sint64, which StreamsForge's generator only emits for envelope 'weight' fields, never entity row fields - treating as Long.");
                return FieldType.Long;
