...
namespace StreamsForge.AppCore.Connectors.Nats;
...
public sealed class NatsInboundTransport(Func<INatsMessageSource>? sourceFactory = null) : IInboundTransport
{
    ...
    public IInboundSubscription Open(SourceDefinition def)
    {
        ...
        var source = sourceFactory?.Invoke() ?? new NatsClientMessageSource($"streamsforge-source-{def.Name}");
    ...
    public TransportDescriptor Describe() => new()
    {
        ...
        [
            ...
            new TransportField
            {
                Key = "queueGroup", Label = "Queue group", Mono = true, Placeholder = "streamsforge-ingest",
