...
/// three lists every caller already has. These are what <see cref="StreamsForge.Abstractions.InstanceInfo"/>'s
...
/// reference <c>StreamsForge.AppCore</c> — a per-flavour copy would be the duplication
...
/// the same method, and <c>ConfigJsonMapper</c> is <c>internal</c> to StreamsForge.AppCore, so a helper
