...
///
/// <para><b>Pure and runtime-free</b> — like the rest of <c>StreamsForge.AppCore/Config</c>, this walks
/// plain data (a <see cref="ConfigDocument"/>) and returns plain data, so it needs no catalog access and
/// no HTTP context. <see cref="StreamsForge.Api"/>'s <c>ConfigImportService.RunImportAsync</c> is the only
...
/// <c>StreamsForge.Contracts.ConnectorModels</c> rather than left as a starting point): a source's
