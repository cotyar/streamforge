...
//
// These interfaces exist so the REST/gRPC surface (StreamsForge.Host.Api/*Endpoints.cs today;
// shared/StreamsForge.Api once wave W3 moves it) can be written once and served by either runtime:
//
...
/// /api/meta/arrangements. Neutral mirror of <c>StreamsForge.Host.Api.ArrangementMetaDto</c>
...
/// <c>StreamsForge.Api</c>, which both flavours serve; without a seam here the route would either not
...
public interface ICrdtFacade
{
    ...
    /// <c>StreamsForge.Connectors.Crdt</c> (it needs Ycs) and <c>StreamsForge.Api</c> deliberately holds no
...
/// <summary>Plan 020 wave D. These two live in Contracts, not in StreamsForge.Connectors.Crdt where the
/// inspector that produces them lives, so that <c>StreamsForge.Api</c> can authorize a decoded update
...
/// <c>StreamsForge.Connectors.Database</c>). The decode itself stays behind
