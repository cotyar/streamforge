...
/// <c>shared/StreamsForge.AppCore.Tests/EndpointImportWarningsTests.cs</c> (that project has no reference
/// to <c>StreamsForge.Api</c>, so the RunImportAsync-level wiring can only be exercised from here).
///
