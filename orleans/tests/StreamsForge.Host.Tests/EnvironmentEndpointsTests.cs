...
/// <see cref="UserAccessEntry"/> — so <see cref="StreamsForge.Api.Auth.PermissionResolver"/> resolves
...
public sealed class EnvironmentEndpointsTests : IAsyncDisposable
{
    ...
    /// legacy-equivalence path (see the class remarks) — no X-StreamsForge-Environment header, so every
