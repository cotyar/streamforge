...
public sealed class DiscoveryEndpointsTests : IAsyncDisposable
{
    ...
    /// hosted service <c>AddStreamForgeApi</c> registers unconditionally, since this test calls
    ...
    private async Task<HttpClient> StartAsync(string flavor = "orleans", IReadOnlyList<string>? grpcServices = null)
    {
        ...
        // AddStreamForgeApi-registered factories for concrete types like AuditChannelSink — stubbing
        ...
        // (a hosted service AddStreamForgeApi registers unconditionally) eagerly constructor-injects
