...
/// <see cref="StreamForge.AppCore.Environments.EnvironmentAmbient"/>, because a hub method invocation runs
...
/// <c>MapStreamForgeApi</c> route table (the <see cref="EnvironmentAmbientTests"/>/
...
/// <c>UseEnvironmentSelection()</c> happens to be called in <c>StreamForgeApiExtensions</c>? An unknown
...
/// route (mapped after <c>MapStreamForgeApi</c>, exactly like <see cref="EnvironmentAmbientTests"/>'s own)
...
public sealed class EnvironmentHubTests
{
    ...
    public sealed class MiddlewareReachesTheHub : IAsyncDisposable
    {
        ...
        private async Task<HttpClient> StartAsync(FakeEnvironmentFacade environments)
        {
            ...
            // Test-only echo route, mapped AFTER MapStreamForgeApi so it still goes through
            // EnvironmentSelectionMiddleware (registered inside MapStreamForgeApi) exactly like every
        ...
        public async Task Negotiate_on_an_unknown_environment_is_the_same_404_every_other_route_gets()
        {
            ...
            // called in StreamForgeApiExtensions".
            using var client = await StartAsync(new FakeEnvironmentFacade("staging"));
