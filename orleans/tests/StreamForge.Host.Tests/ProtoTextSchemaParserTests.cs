...
/// then separately proves a non-StreamForge (or stripped-banner) proto is rejected with a diagnostic
...
public class ProtoTextSchemaParserTests
{
    ...
    public void Long_and_timestamp_fields_are_disambiguated_via_the_generator_comment_not_the_bare_int64_keyword()
    {
        ...
        // ambiguity). Confirms the parser actually reads the trailing "// StreamForge: ..." comment.
        ...
        Assert.Contains("int64 qty = 1; // StreamForge: Long", protoText);
        Assert.Contains("int64 as_of = 2; // StreamForge: Timestamp (epoch milliseconds)", protoText);
    ...
    [Fact]
    public void A_hand_written_proto_without_the_StreamForge_banner_is_rejected_with_a_diagnostic()
    {
        ...
        Assert.Null(fields);
        Assert.Null(numbers);
        Assert.Contains(diagnostics, d => d.Contains("StreamForge-generated", StringComparison.OrdinalIgnoreCase));
    }
    ...
    [Fact]
    public void A_StreamForge_banner_with_a_foreign_package_is_rejected_with_a_diagnostic()
    {
        const string tampered = """
            // StreamForge generated .proto — do not edit by hand.
