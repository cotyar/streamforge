...
- **Aggregates**: `COUNT(*) | COUNT(x) | SUM | AVG | MIN | MAX`. **Functions**: `ABS, ROUND, UPPER, LOWER, COALESCE`. Full expression grammar with `AND/OR/NOT`, comparisons, arithmetic, SQL three-valued NULL semantics; `/` always yields a double.
