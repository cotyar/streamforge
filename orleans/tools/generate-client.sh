#!/usr/bin/env bash
#
# generate-client.sh — StreamForge Tier-3 typed client codegen.
#
# Downloads the self-contained .proto for one StreamForge source/pipeline/table and scaffolds a
# ready-to-build .NET client library project around it: a .csproj that compiles the downloaded .proto
# with Grpc.Tools (GrpcServices=Client), and a small StreamForgeClient.cs convenience wrapper that logs
...
#
# Options:
#   --server <url>    StreamForge REST base URL (default: http://localhost:5199)
...
# dotnet isn't on PATH in the StreamForge dev environment — prefer the well-known location, falling
...
# Every StreamForge dynamic entity gets both an {Entity}Event and an {Entity}Delta message (see
...
# C# namespace protoc generates for `package streamforge.dynamic.v1;`: each dot-segment gets its
# first letter capitalized ("streamforge" -> "Streamforge", "v1" -> "V1"). Derived from the actual
...
cat > "${OUT_DIR}/StreamForgeClient.cs" <<EOF
...
public static class StreamForgeClient
{
    ...
    public static async IAsyncEnumerable<T> SubscribeTypedAsync<T>(
        ...
        where T : IMessage<T>
    {
        // StreamForge's gRPC endpoint is cleartext h2c (HTTP/2 without TLS) — SocketsHttpHandler
...
echo "    Subscribe with: StreamForgeClient.SubscribeAsync(grpcBaseUrl, jwt)"
