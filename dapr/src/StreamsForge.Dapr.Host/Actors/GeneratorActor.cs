...
/// (orleans/src/StreamsForge.Host/Grains/GeneratorGrain.cs) — one actor per source (actor type
...
/// cadence, StartAsync attaches this activation to <c>LoopbackHub</c> (shared/StreamsForge.AppCore/
    ...
    : Actor(host), IGeneratorActor
{
    ...
    /// of Orleans' <c>GeneratorGrain.RunAsync</c> (orleans/src/StreamsForge.Host/Grains/GeneratorGrain.cs).
    /// Delegates the whole spec/request-validation + deterministic row-math decision to the pure, TOTAL
    /// <c>ScenarioGenerator</c> (shared/StreamsForge.AppCore/Generators/ScenarioGenerator.cs — the exact
...
/// tested without any actor/timer/Dapr-sidecar machinery (see dapr/tests/StreamsForge.Dapr.Tests/
