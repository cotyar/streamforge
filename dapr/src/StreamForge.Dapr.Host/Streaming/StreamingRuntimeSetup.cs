...
public static class StreamingRuntimeSetup
{
    ...
    public static void MapTopicEndpoints(WebApplication app)
    {
        var logger = app.Services.GetRequiredService<ILoggerFactory>()
            .CreateLogger("StreamForge.Dapr.Host.Streaming.TopicEndpoints");
    ...
    /// endpoint in StreamForge.Api binds with — camelCase output, case-INsensitive input, per
    /// StreamForgeApiExtensions.AddStreamForgeApi's <c>ConfigureHttpJsonOptions</c> call), so PascalCase
