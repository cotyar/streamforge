...
/// (orleans/src/StreamForge.Host/Grains/PipelineGrain.cs) — one actor per running pipeline (actor type
/// "PipelineActor", key = the pipeline's <see cref="PipelineDefinition.Id"/>), compiling the pipeline's
/// streaming SQL via the shared <see cref="StreamForge.Engine"/> and feeding it batches of source events
...
/// dapr/tests/StreamForge.Dapr.Tests/PipelineCompilationTests.cs. Builds the same schema dictionary +
