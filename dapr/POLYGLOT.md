# StreamForge — Dapr polyglot pub/sub contract (plan 005, wave W5-B)
...
(`shared/StreamForge.Api/StreamForgeApiExtensions.cs`'s `ConfigureHttpJsonOptions` call only adds a
...
- `events` (array of objects): each object is a free-form field-name → value map — this is the row shape
  ...
  `_ts`/`_source` are honored the same way `StreamForge.Engine.EventRecord`'s own accessors read them
...
`orleans/src/StreamForge.Host/Services/StreamBridgeService.cs`'s doc comment on `OnLifecycleEventAsync`).
...
`StreamForge.AppCore.Json.JsonValueNormalizer.NormalizeInPlace` immediately after deserialization, before
...
```bash
dapr publish --publish-app-id streamforge-dapr --pubsub pubsub --topic sf-sources --data \
  '{"source":"trades","events":[{"symbol":"AAPL","price":101.5,"qty":10}]}'

dapr publish --publish-app-id streamforge-dapr --pubsub pubsub --topic sf-table-delta --data \
  '{"table":"positions","seq":1,"deltas":[{"row":{"symbol":"AAPL","qty":5},"weight":1}]}'

dapr publish --publish-app-id streamforge-dapr --pubsub pubsub --topic sf-lifecycle --data \
  '{"pipelineId":"p1","kind":"started","status":"Running","timestampMs":0}'

dapr publish --publish-app-id streamforge-dapr --pubsub pubsub --topic sf-metrics --data \
...
## SignalR relay (for reference — see `dapr/src/StreamForge.Dapr.Host/Streaming/DaprStreamBridge.cs`)
