# @streamsforge/tanstack-db

A [TanStack DB](https://tanstack.com/db) collection backed by a StreamsForge live table.

**Layering** -- read this before anything else, because it's easy to conflate the pieces:

| Layer | Does |
| --- | --- |
| StreamsForge **server** | Runs the actual incremental-view-maintenance (IVM) dataflow and ships Z-set deltas over the wire. |
| `@streamsforge/client`'s **`LiveTable`** | The client-side reducer: folds those deltas into current rows (`zset.ts`). |
| `@streamsforge/tanstack-db` (**this package**) | A thin bridge, nothing more: relays `LiveTable`'s already-reduced rows into a TanStack DB `Collection`. Adds no reduction logic of its own. |
...
```bash
bun add @streamsforge/tanstack-db @tanstack/db @streamsforge/client
bun run build
```

`@tanstack/db` and `@streamsforge/client` are peer dependencies -- the host app owns their versions.
...
import { streamsForgeCollectionOptions } from "@streamsforge/tanstack-db";

const sf = await connect({ url: "http://localhost:5199" });

const orders = createCollection(
  streamsForgeCollectionOptions({ client: sf, table: "orders" }),
);
...
StreamsForge until something actually reads from the collection (`.preload()`, a live query that
...
### A live query joining two StreamsForge tables

This is the payoff: once two StreamsForge tables are each their own collection, TanStack DB's own
...
const orders = createCollection(streamsForgeCollectionOptions({ client: sf, table: "orders" }));
const desks = createCollection(streamsForgeCollectionOptions({ client: sf, table: "desk_exposure" }));
...
`ordersWithDesk` re-derives incrementally as either source collection changes -- StreamsForge did
...
```ts
streamsForgeCollectionOptions({
  client,     // a connected @streamsforge/client Client
...
by `@streamsforge/client`). This is deliberate and non-negotiable: it is NOT a column this package
...
```ts
// ponytail: StreamsForge writes go through the INGEST path (client.push(source, rows)), which
...
If a host app wants optimistic writes on top of a StreamsForge-backed collection, that layer has to
...
Earlier versions of `@streamsforge/client`'s `ZSet.apply()` only ever pushed *asserted* keys into
...
`@streamsforge/client`'s `zset.ts` `apply()` doc comment for the exact contract). This package no
...
`test/streamsforge-collection.test.ts` drives a real `Client`/`LiveTable` against a hand-rolled fake
