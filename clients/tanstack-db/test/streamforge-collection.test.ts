/**
 * Drives `streamForgeCollectionOptions` with a real `Client`/`LiveTable` but a hand-rolled fake
...
import { Client, canonicalKey, type Delta } from "@streamforge/client";
import { streamForgeCollectionOptions } from "../src/index.js";
import type { Transport } from "@streamforge/client";
...
/** `RestClient` isn't part of `@streamforge/client`'s public surface (its `package.json` `exports`
...
describe("streamForgeCollectionOptions", () => {
  test("seeds the collection from the initial snapshot as inserts, and reports ready only after", async () => {
    ...
    const collection = createCollection(streamForgeCollectionOptions({ client, table: "t", key: ["id"] }));
  ...
  test("a retraction deletes the row from the collection", async () => {
    ...
    // @streamforge/client's zset.ts `apply()` doc comment), so this needs no same-window coincidence
