 ...
 * lib/streamsforge/server.ts. Always REST regardless of which live transport a Client picked --
 * these are cheap, infrequent calls (ported from clients/python/src/streamsforge/tables.py).
 */
...
export async function listTables(http: RestClient): Promise<TableDefinitionDto[]> {
  ...
  if (!res.ok) throw new StreamsForgeError(`GET /api/tables failed: ${res.status} ${await res.text()}`);
...
export async function resolveTableId(http: RestClient, name: string): Promise<string> {
  ...
  if (t === null) throw new StreamsForgeError(`no such table '${name}'`);
  return t.id;
}
...
export async function search(http: RestClient, name: string, query: string, limit = 50): Promise<Row[]> {
  ...
  if (!res.ok) throw new StreamsForgeError(`search '${name}' failed: ${res.status} ${await res.text()}`);
...
export async function history(http: RestClient, name: string, row: Row, limit?: number): Promise<unknown[]> {
  ...
  if (!res.ok) throw new StreamsForgeError(`history '${name}' failed: ${res.status} ${await res.text()}`);
...
export async function snapshotDeltas(http: RestClient, name: string, limit = 500): Promise<readonly [Delta[], number]> {
  ...
  if (!res.ok) throw new StreamsForgeError(`rows '${name}' failed: ${res.status} ${await res.text()}`);
