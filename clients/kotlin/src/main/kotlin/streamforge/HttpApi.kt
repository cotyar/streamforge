...
/**
 * REST client with cached, self-refreshing StreamForge auth. Ported from `lib/streamforge/
 ...
 * token -- if THAT also 401s, this raises rather than looping forever (a StreamForge restart
...
class AuthClient(
    ...
    private fun loginLocked() {
        if (user.isNullOrBlank() || password.isNullOrBlank()) {
            throw AuthException(
                "no StreamForge credentials configured -- pass user=/password= to StreamForge.connect()"
            )
        }
        ...
        if (response.statusCode() != 200) {
            throw AuthException("StreamForge login failed: ${response.statusCode()} ${response.body()}")
        }
        val json = gson.fromJson(response.body(), com.google.gson.JsonObject::class.java)
        cachedToken = json.get("token")?.asString
            ?: throw AuthException("StreamForge login response had no 'token' field")
    ...
    suspend fun request(
        ...
        if (response.statusCode() == 401) {
            ...
            if (response.statusCode() == 401) {
                throw AuthException("StreamForge rejected the re-minted token for $method $path")
            }
        }
        response
    }
