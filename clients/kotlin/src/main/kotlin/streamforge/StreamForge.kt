...
private val logger = Logger.getLogger("streamforge")

/** Which live transport [StreamForge.connect] uses. `AUTO` tries gRPC, falls back to SignalR,
 ...
 * `StreamForge.connect(transport = Transport.GRPC)` is meant to read as.) */
...
object StreamForge {
     ...
     * @param ingestKey `X-SF-Ingest-Key`, preferred over the admin JWT for [StreamForgeClient.push]
    ...
    suspend fun connect(
        ...
        if (transport == Transport.GRPC || transport == Transport.AUTO) {
            ...
            try {
                ...
                logger.warning(
                    "streamforge: gRPC unavailable (${e.javaClass.simpleName}: ${e.message}), falling back to SignalR"
                )
            }
        }
        ...
        logger.info("streamforge: connected via $chosen transport ($url)")

        val scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
        return StreamForgeClient(http, grpc, liveTransport, ingestKey, chosen, scope)
    }
...
/**
 * Returned by [StreamForge.connect]. Owns the REST auth client, the chosen live [TableTransport]
 ...
 */
class StreamForgeClient internal constructor(
