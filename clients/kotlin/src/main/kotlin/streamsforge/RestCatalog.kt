...
internal object RestCatalog {
    ...
    private fun checkOk(response: java.net.http.HttpResponse<String>, what: String) {
        if (response.statusCode() >= 400) {
            throw StreamsForgeError("$what failed: ${response.statusCode()} ${response.body()}")
        }
    }
    ...
    suspend fun resolveTableId(http: AuthClient, name: String): String {
        val table = listTables(http).firstOrNull { it["name"] == name }
            ?: throw StreamsForgeError("no such table '$name'")
        return table["id"] as? String ?: throw StreamsForgeError("table '$name' has no id")
    }
     ...
     * `null` default in [StreamsForgeClient.table]), and now it resolves to whatever the engine
     ...
     * `lib/streamsforge/server.ts` applies. This backs [SignalRTransport]'s [TableTransport.
     * snapshot] AND the public `StreamsForgeClient.snapshot()` one-shot read. */
     ...
     * streamsforge/adhoc.ts`'s `adhocTableName`). */
    ...
    suspend fun dropAdhoc(http: AuthClient, name: String): Boolean {
        if (!name.startsWith(ADHOC_PREFIX)) throw StreamsForgeError("refusing to drop non-ad-hoc table '$name'")
        ...
        if (resp.statusCode() != 200 && resp.statusCode() != 204) {
            throw StreamsForgeError("drop '$name' failed: ${resp.statusCode()} ${resp.body()}")
        }
        return true
    }
