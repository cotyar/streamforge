"""httpx-based REST client with cached, self-refreshing StreamsForge auth.

Ported from lib/streamsforge/server.ts's sfFetch: the JWT is cached in memory for ~11h (the
...
StreamsForge restart invalidates every token minted before it, which is a normal event, but an
...
class AuthClient:
    ...
    def _login_locked(self) -> None:
        if not self._user or not self._password:
            raise AuthError(
                "no StreamsForge credentials configured -- pass user=/password= to connect(), "
                "set STREAMSFORGE_ADMIN_USER/STREAMSFORGE_ADMIN_PASS, or add them to "
        ...
        if resp.status_code != 200:
            raise AuthError(f"StreamsForge login failed: {resp.status_code} {resp.text}")
        body = resp.json()
    ...
    def request(self, method: str, path: str, *, auth: bool = True, **kwargs) -> httpx.Response:
        ...
        if resp.status_code == 401:
            ...
            resp = self._client.request(method, url, headers=headers, **kwargs)
            if resp.status_code == 401:
                raise AuthError(f"StreamsForge rejected the re-minted token for {method} {path}")
        return resp
