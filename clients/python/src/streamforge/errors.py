...
Everything below subclasses StreamForgeError, so `except StreamForgeError` catches the whole
...
class AuthError(StreamForgeError):
    """Login failed, or a 401 survived the one-shot re-mint.

    Mirrors sfFetch in lib/streamforge/server.ts: the cached token is discarded and re-minted
    exactly once on a 401; if the retry also 401s, this is raised rather than looping.
    """


class NotReady(StreamForgeError):
...
class IngestRejected(StreamForgeError):
...
class SqlError(StreamForgeError):
