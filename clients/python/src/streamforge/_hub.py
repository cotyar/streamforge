...
def _handshake_and_subscribe(pipe, table_name: str) -> Iterator[str]:
    ...
    for frame in frames:
        ...
        if msg.get("type") == 3 and msg.get("invocationId") == _SUB_ID:
            if msg.get("error"):
                raise StreamForgeError(f"SubscribeTable('{table_name}') rejected: {msg['error']}")
            break
        early.append(frame)
    else:
        raise StreamForgeError(f"connection closed before SubscribeTable('{table_name}') completed")
...
def _negotiate(http) -> str:
    ...
    if not token:
        raise StreamForgeError("SignalR negotiate returned no connectionToken")
    return token
...
class HubTransport:
    def __init__(self, http, mode: str = "ws", verify: bool = True) -> None:
        if mode not in _MODES:
            raise StreamForgeError(f"unknown signalr mode '{mode}' -- expected one of {_MODES}")
        self._http = http
        self._mode = mode
        self._verify = verify
