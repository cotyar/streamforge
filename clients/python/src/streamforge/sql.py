...
are ported from lib/streamforge/adhoc.ts (otc-terms) -- see runAdhocQuery/adhocTableName/
...
from .errors import SqlError, StreamForgeError
...
def drop_adhoc(http, name: str) -> bool:
    ...
    if not name.startswith(ADHOC_PREFIX):
        raise StreamForgeError(f"refusing to drop non-ad-hoc table '{name}'")
    ...
    if resp.status_code not in (200, 204):
        raise StreamForgeError(f"drop '{name}' failed: {resp.status_code} {resp.text}")
    return True
