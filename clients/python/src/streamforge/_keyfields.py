...
lib/streamforge/catalog.ts's `keyFields` (otc-terms). `sf.table(name, key=[...])` always
overrides; this only fills in when a caller omits `key=` for a table this map happens to know
about. Unknown-and-omitted falls back to whole-row identity in `_zset.group_key_of`, never a
guessed first column.

This is copy #4 of the same hand-maintained list (the web console's per-call-site keyFields, the
Excel add-in's KEY_FIELDS, whatever a /sql user types into the editor's key box) -- wishlist #18
(surface TableGroupKeyExtractor.Describe's result on TableDefinition) is the fix that deletes all
four. Until then, this file is demo-shaped: it only knows the otc-terms reference catalog's
tables. A StreamForge instance running a different SQL catalog gets no fallback here and falls
through to whole-row identity, which is correct (if occasionally deduplication-free) rather than
silently wrong.
"""
