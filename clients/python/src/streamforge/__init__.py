"""streamforge -- sync-first Python client for StreamForge live tables.

    import streamforge
    sf = streamforge.connect()          # env / ~/.config/streamforge/config.toml
...
def _probe_signalr_mode(http: AuthClient, verify: bool) -> str:
    ...
    try:
        ...
        logger.warning("streamforge: signalr:ws unavailable (%s), trying signalr:sse", exc)
    ...
    try:
        ...
        logger.warning("streamforge: signalr:sse unavailable (%s), trying signalr:lp", exc)
...
def connect(
    ...
    (STREAMFORGE_BASE_URL / STREAMFORGE_GRPC / STREAMFORGE_ADMIN_USER / STREAMFORGE_ADMIN_PASS) ->
    ...
    if not cfg.base_url:
        raise StreamForgeError(
            "no base URL: pass url=, set STREAMFORGE_BASE_URL, or add base_url to "
            "~/.config/streamforge/config.toml"
        )
    ...
    if transport in ("grpc", "auto"):
        ...
        try:
            ...
            logger.warning(
                "streamforge: gRPC unavailable (%s: %s), falling back to SignalR", type(exc).__name__, exc
            )
    ...
    logger.info("streamforge: connected via %s transport (%s)", chosen, cfg.base_url)
