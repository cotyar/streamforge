...
namespace StreamForge.Client.Tests.Fixtures;

/// <summary>
/// Contract-test fixture: boots an ISOLATED StreamForge instance on 9199/9299 (never 5199/5299 --
...
public sealed class EngineFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        {
            SkipReason = $"StreamForge.Host project not found at {projectDir}";
            return;
        }
        ...
        else if (!File.Exists(Path.Combine(_publishDir, "StreamForge.Host.dll")))
        {
            SkipReason = $"SF_TEST_PUBLISH_DIR={_publishDir} has no StreamForge.Host.dll";
            return;
        }
    ...
    private static string ProjectDir([CallerFilePath] string thisFile = "") =>
        Path.GetFullPath(Path.Combine(Path.GetDirectoryName(thisFile)!, "..", "..", "..", "..", "orleans", "src", "StreamForge.Host"));
    ...
    {
        var dll = Path.Combine(publishDir, "StreamForge.Host.dll");
        ...
        _process = Process.Start(psi) ?? throw new InvalidOperationException("failed to start StreamForge.Host");
