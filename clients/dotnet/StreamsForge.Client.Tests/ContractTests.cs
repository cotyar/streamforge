...
/// <summary>
/// Contract tests against a real, isolated StreamsForge instance (see <see cref="EngineFixture"/>),
...
public sealed class ContractTests
{
    ...
    private Task<StreamsForgeClient> ConnectAsync(TransportKind kind) =>
        StreamsForgeClient.ConnectAsync(new ConnectOptions
        {
            Url = _engine.BaseUrl,
    ...
    public async Task SnapshotMatchesLiveTableRows(TransportKind kind)
    {
        ...
        // SnapshotAsync is a plain REST read (see StreamsForgeClient's class doc: catalog reads are
    ...
    public async Task SqlAsyncCreatesQueriesAndDropsAnAdhocTable()
    {
        ...
        var adhocName = StreamsForgeClient.AdhocTableName(rawName);
        ...
        await Assert.ThrowsAsync<StreamsForgeException>(() => client.DropAdhocAsync("not_prefixed"));
    }
}
