...
/// <summary>
/// REST client with cached, self-refreshing StreamForge auth. Ported from clients/python/src/streamforge/_http.py:
...
/// raises rather than looping forever (a StreamForge restart invalidates every token minted
...
internal sealed class AuthHttpClient : IAsyncDisposable
{
    ...
    private async Task LoginLockedAsync(CancellationToken ct)
    {
        if (string.IsNullOrEmpty(_user) || string.IsNullOrEmpty(_password))
        {
            throw new AuthException(
                "no StreamForge credentials configured -- set ConnectOptions.User/Password or " +
                "the STREAMFORGE_ADMIN_USER/STREAMFORGE_ADMIN_PASS environment variables");
        }
        ...
        if (!resp.IsSuccessStatusCode)
        {
            ...
            throw new AuthException($"StreamForge login failed: {(int)resp.StatusCode} {body}");
        }
        ...
        Action<HttpRequestMessage>? configure = null)
    {
        ...
        if (retryResp.StatusCode == HttpStatusCode.Unauthorized)
            throw new AuthException($"StreamForge rejected the re-minted token for {method} {path}");
        return retryResp;
    }
