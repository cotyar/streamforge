...
/// simplification from clients/python/src/streamforge/tables.py / sql.py: these are cheap,
...
/// </summary>
public sealed class StreamForgeClient : IAsyncDisposable
{
    ...
    private StreamForgeClient(AuthHttpClient http, GrpcTransport? grpc, ITransport live, string? ingestKey, string transportName, ILogger logger)
    {
        _http = http;
        _grpc = grpc;
        _liveTransport = live;
    ...
    public static async Task<StreamForgeClient> ConnectAsync(ConnectOptions options, CancellationToken ct = default)
    {
        var url = options.Url ?? Environment.GetEnvironmentVariable("STREAMFORGE_BASE_URL");
        if (string.IsNullOrEmpty(url))
            throw new StreamForgeException("no base URL: set ConnectOptions.Url or STREAMFORGE_BASE_URL");

        var user = options.User ?? Environment.GetEnvironmentVariable("STREAMFORGE_ADMIN_USER");
        var password = options.Password ?? Environment.GetEnvironmentVariable("STREAMFORGE_ADMIN_PASS");
        var ingestKey = options.IngestKey ?? Environment.GetEnvironmentVariable("SF_INGEST_KEY");
        var logger = (options.LoggerFactory ?? NullLoggerFactory.Instance).CreateLogger("StreamForge");
        ...
        {
            var target = options.GrpcTarget ?? Environment.GetEnvironmentVariable("STREAMFORGE_GRPC") ?? DefaultGrpcTarget(url);
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                logger.LogWarning(ex, "streamforge: gRPC unavailable, falling back to SignalR");
            }
        }
        ...
        logger.LogInformation("streamforge: connected via {Transport} transport ({Url})", chosen, url);

        return new StreamForgeClient(http, grpc, live, ingestKey, chosen, logger);
    }
    ...
    private static async Task<ITransport> ResolveSignalRAsync(string url, AuthHttpClient http, ConnectOptions options, ILogger logger, CancellationToken ct)
    {
        ...
        foreach (var (flag, label) in SignalRPriority)
        {
            ...
            {
                logger.LogWarning(ex, "streamforge: signalr:{Mode} unavailable, trying next", label);
            }
        }
        throw new StreamForgeException("no SignalR transport in the requested set could connect");
    }
    ...
    public async Task<bool> DropAdhocAsync(string name, CancellationToken ct = default)
    {
        if (!name.StartsWith(AdhocPrefix, StringComparison.Ordinal))
            throw new StreamForgeException($"refusing to drop non-ad-hoc table '{name}'");
        ...
        if (resp.StatusCode is not (HttpStatusCode.OK or HttpStatusCode.NoContent))
        {
            ...
            throw new StreamForgeException($"drop '{name}' failed: {(int)resp.StatusCode} {body}");
        }
        return true;
    }
