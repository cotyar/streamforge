...
using StreamForge.Host.Grpc.V1;
...
internal sealed class GrpcTransport : ITransport, IAsyncDisposable
{
    ...
    /// many pushes) is future work, not needed for <see cref="StreamForgeClient.PushAsync"/>.</summary>
        ...
        string source, IReadOnlyList<IReadOnlyDictionary<string, object?>> rows, string? idempotencyKey, bool partial, CancellationToken ct)
    {
        ...
        if (!await call.ResponseStream.MoveNext(ct).ConfigureAwait(false))
            throw new StreamForgeException($"gRPC Ingest('{source}') stream closed with no ack");
        ...
        // should not leak into StreamForgeClient's transport-agnostic error handling -- REST's
